...
def drawSingleWithCustomId(custom,sessionId,callback):
    print("DRAW SINGLE WITH CUSTOM ID")
    headers = {
        "Authorization" : "NzA5MzM0MjI1NjM1MjQ2MDky.GBRHSW.ehL9jxAqUYPIrfZpdYn_yRa5UMquDLaWVsdfcE"
    }
    ...
    if isinstance(custom, CustomIdObject):
        ...
        str = custom.customIds
        customid_array = json.loads(str)
        customid_array_0 = customid_array[0]
        print(customid_array_0)
        choosen = ""
        if isinstance(customid_array_0,list):
            if len(customid_array_0) == 5:
                index = random.randint(0,3)
                choosen = customid_array_0[index]
        print(f"CUSTOM ID = {choosen}")
        if choosen == "":
            return
        else:
            data = {
